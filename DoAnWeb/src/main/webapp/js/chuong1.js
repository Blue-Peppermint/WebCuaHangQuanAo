...
if (window.location.href.includes('/home/index.htm') || window.location.href == 'http://localhost:8080/DoAnWeb/') {
	// ******************************************* Test1.html

	/*	jQuery(function(){
			var valuePassedFromJSP = "<%=myVarB%>"
	//$("body").attr("data-customvalueone");
    ...
    `;
				...
				if (btnActivatedSpecialMenu.getAttribute('my-data-filter') === 'discount') {
					...
					console.log(bodyEle.getAttribute('json-discount-p'))
					var jsonDis = bodyEle.getAttribute('json-discount-p')


					console.log(jsonDis)
					//items = JSON.parse(jsonDis);
					//console.log(items)
				} else if (btnActivatedSpecialMenu.getAttribute('my-data-filter') === 'wishlist') {
					console.log("di vao wishlist")
					items = JSON.parse($("body").attr("jsonWishlistProducts"));
				}
				else if (btnActivatedSpecialMenu.getAttribute('my-data-filter') === 'best-seller') {
					items = JSON.parse($("body").attr("jsonBestSellingProducts"));
				}
				for (var i = 0; i < 4; i++) {
	...
	function loadMoreItems(btnName) {
		...
		switch (btnName) {
			case 'discount':
				//itemLoaded += 4
				items = JSON.parse($("body").attr("jsonDiscountProducts"));
...
`;
	...
	function addItemToDiv(div, item) {
		...
		divItemAdding1.innerHTML += `
                <img src="${item.imgSources[0]}" class="img-fluid" alt="Image">
                `;
	...
	} else if (window.location.href.includes('shop-detail.htm')) {
