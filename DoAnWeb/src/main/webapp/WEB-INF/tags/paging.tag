...
<c:if test="${pagedListHolder.pageCount > 1}">
	<ul class="pagination justify-content-end">
		<c:if test="${!pagedListHolder.firstPage}">
			<li class="page-item"><a class="btn hvr-hover myhover page-link"
				href="<%=StringUtils.replace(pagedLink, "~", String.valueOf(pagedListHolder.getPage() - 1))%>">Previous</a></li>
		</c:if>
		<c:if test="${pagedListHolder.firstLinkedPage > 0}">
			<li class="page-item"><a class="btn hvr-hover myhover page-link" href="<%=StringUtils.replace(pagedLink, "~", "0")%>">1</a></li>
		</c:if>
		...
		<c:forEach begin="${pagedListHolder.firstLinkedPage}"
			...
			<c:choose>
				<c:when test="${pagedListHolder.page == i}">
					<li class="page-item active"><a class="btn hvr-hover myhover page-link" href="#">${i+1}</a></li>
				</c:when>
				<c:otherwise>
					<li><a class="btn hvr-hover myhover page-link"
